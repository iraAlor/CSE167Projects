...
void Window::initialize(void)
{
    ...
    delete Globals::man.getObject("bear.obj");
    delete Globals::man.getObject("bunny.obj");
    delete Globals::man.getObject("dragon.obj");
    Globals::man.addToMode(1,new House());
    Globals::house = House();
...
void Window::reshapeCallback(int w, int h)
{
    ...
    if(Globals::engine){
      ...
      gluPerspective(60.0, double(width)/(double)height, 1.0, 1000.0);
        ...
        std::cout<<"j\n";
      Globals::port.update(w,h);
      Globals::projection.update(60,double(width)/ (double)height,1.0,1000.0);
...
void Window::displayCallback()
{
    ...
    if(Globals::engine){
     ...
     glFlush();
        ...
        Globals::pixels.clear();
      //  std::cout<<"here\n";
        if(Globals::pixels.rastMode == 0){
            Globals::man.nonGlutRasterV(Globals::pixels,Globals::projection.projection*Globals::camera.getInverseMatrix(),Globals::port.getMatrix(),(&Canvas::rasterVetices));
        }
        else{
            Globals::man.nonGlutRasterV(Globals::pixels,Globals::projection.projection*Globals::camera.getInverseMatrix(),Globals::port.getMatrix(),(&Canvas::rasterTriangles));
        }
       // Globals::pixels.rasterVetices(obj,,Globals::port.getMatrix());
       // std::cout<<"blue\n";
        glDrawPixels(width, height, GL_RGB, GL_FLOAT, Globals::pixels.getArr());
...
void Window::processKeys(unsigned char key,int x,int y){
    if(key == 'e'){
        updateEngine(Globals::engine);
        Globals::engine = !Globals::engine;
        
    }
    if(key == '+'){
        Globals::pixels.rastMode++;
    }
    if(key == '-'){
        Globals::pixels.rastMode--;
    ...
    if(key == 'd'){
        Globals::pixels.debug = !Globals::pixels.debug;
    }
    if(Globals::mode){
        nonCamMode(key,x,y);
    }
    ...
    switch(key){
        ...
        case GLUT_KEY_F4:
            ...
            if(Globals::project != 2){
                Globals::man.setMode(2,"bunny.obj");
            }
            else{
        ...
        case GLUT_KEY_F5:
            if(Globals::project != 4){
                Globals::man.setMode(4,"dragon.obj");
            }
            else{
        ...
        case GLUT_KEY_F6:
            if(Globals::project != 6){
                ...
                Globals::man.setMode(6,"bear.obj");
            }
            else{
