...
void OBJObjectMan::applyTransForm(Matrix4 tform){
    std::vector<Drawable*> * cvect =  dmap[mode];
    if(cvect->size()<=0){
        return;
    }
    cvect->back()->toWorld = tform * cvect->back()->toWorld;
...
void OBJObjectMan::nonGlutRasterV(Canvas can,Vector3 e,Matrix4 projection,Matrix4 transform, Matrix4 & port,void (Canvas::*fptr)(OBJObject *,Vector3,Matrix4,Matrix4,Matrix4&)){
    std::vector<Drawable*> * cvect =  dmap[mode];
    if(cvect->size()<=0){
        return;
    }
    // cvect->back()->toWorld.print("ll ");
    (can.*fptr)((OBJObject*)cvect->back(),e,projection*cvect->back()->toWorld,transform,port);
    
}

void OBJObjectMan::nonGlutRasterV(Canvas can,Matrix4 transform, Matrix4 & port,void (Canvas::*fptr)(OBJObject *,Matrix4,Matrix4&)){
    std::vector<Drawable*> * cvect =  dmap[mode];
    if(cvect->size()<=0){
        return;
    }
   // cvect->back()->toWorld.print("ll ");
   (can.*fptr)((OBJObject*)cvect->back(),transform*cvect->back()->toWorld,port);
